...
CodeMirror.defineOption('sparqlSupportQueries', false, function (cm: CodeMirror.Editor, id: string) {
  let data = cm.state.selectionPointer
...
CodeMirror.defineOption('sparqlSupportInnerMode', false, function (cm: CodeMirror.Editor, id: string) {
  let data = cm.state.selectionPointer
...
function keyDown(cm, e, id) {
  ...
  const selTab = parseInt(localStorage[ssParam.pathName + '_sparql_code_select_tab_' + id])
  //	console.log("'" + e.key + "'" + " " + e.code);
  if (ssParam.confirmFlag && !e.ctrlKey && e.key != 'Shift') {
  ...
  if (e.key == 'Tab' || (e.key == ' ' && e.ctrlKey)) {
    ...
    const tmp = tabKeyDown(cm, caret, id, e.shiftKey)
    const string = tmp[0]
    const prefixCopy = tmp[1]
    ...
    // Ctrl+Enter: decision (submit query, remove query, command...)
    const line = cm.getLine(caret.line)
    if (line.match(/^#.+;\s*$/)) {
  ...
  } else if (e.ctrlKey && parseInt(e.key) >= 1 && parseInt(e.key) <= 9) {
    // Ctrl+[1-9]: move tab
    changeTab(cm, parseInt(e.key) - 1, id)
  } else if (e.ctrlKey && e.key == 'Shift') {
    // Ctrl+Shift(+Alt): move neighbor tab
    ...
    if (ssParam.preString.match(/^\w+:\w+$/)) {
      if (ssParam.freqTerm[ssParam.preString]) {
        ssParam.freqTerm[ssParam.preString]++
      } else {
        ssParam.freqTerm[ssParam.preString] = 1
      }

      if (ssParam.freqTermPre.length > 3000) {
        ssParam.freqTermPre.shift().push(ssParam.preString)
      } else {
        ssParam.freqTermPre.push(ssParam.preString)
      }
  ...
  if (e.target.className.match('query_tab')) {
    if (e.target.id == 'query_tab_plus_' + id) {
      ...
      if (parseInt(e.target.innerHTML) - 1 != selTab) changeTab(cm, parseInt(e.target.innerHTML) - 1, id)
        ...
        const button = document.getElementById('query_tab_0_' + id)
        const baseStyle = button.currentStyle || document.defaultView.getComputedStyle(button, '')
        const width = baseStyle.width.replace('px', '') - 0
        const margin = baseStyle.marginRight.replace('px', '') - 0
...
function mouseMove(cm, e, id) {
  if (ssParam.dragFlag) {
    const selTab = parseInt(localStorage[ssParam.pathName + '_sparql_code_select_tab_' + id])
    const tabNum = parseInt(localStorage[ssParam.pathName + '_sparql_code_tab_num_' + id])
    const dist = e.pageX - ssParam.mouseXstart
...
function mouseDownInner(e, id) {
  ...
  if (e.target.id == 'submit_button_' + id && ssParam.innerMode[id]) {
    ...
    switchInnerMode(id)
  } else if (e.target.className == 'describe' && e.which == 1) {
    // left click
    const uri = decodeURIComponent(e.target.href)
    e.preventDefault()
...
function setCmDiv(cm, id) {
  const text = ssParam.textarea[id].value
...
function tabKeyDown(cm, caret, id, shiftKey) {
  ...
  if (
    caret.line == 0 &&
    ((ssParam.queryFormer.match(/^##+$/) && ssParam.termFrag.match(/^##+$/)) ||
      (ssParam.queryFormer.match(/^##+ *$/) && ssParam.termFrag.match(/^endpoint$/)) ||
      (ssParam.queryFormer.match(/^##+ *endpoint $/) && ssParam.termFrag.match(/^http$/)))
  ) {
    string = autoCompletionEndpoint()
  } else if (!ssParam.queryFormer && ssParam.termFrag.match(/^C$/) && caret.line == 0) {
    string = 'COPY'
  } else if (!ssParam.queryFormer && ssParam.termFrag.match(/^D[Ee][Ff]$/)) {
    string = 'DEFINE sql:select-option "order"'
  } else if (!ssParam.termFrag) return [false, false]
  else {
    const tmp = autoCompletion(cm, caret, id, shiftKey)
    string = tmp[0]
    prefixCopy = tmp[1]
  }
  ...
  function getTermFrag(cm, caret) {
    const lineString = cm.getLine(caret.line)
    ssParam.queryFormer = lineString.substr(0, caret.ch).replace(/( *)([\(\{\[]*)\<*[\w:\?\$]*\>*$/, '$1$2')
    ssParam.queryLatter = lineString.substr(caret.ch)
    if (
      !lineString.substr(caret.ch, 1) ||
      lineString.substr(caret.ch, 1).match(/^[\s\/\^\,\.\;\"\'\(\[\{\<\)\]\}\>]/)
    ) {
      const tmpTerms = lineString.substr(0, caret.ch).split(/[\s\/\^\,\.\;\"\'\(\[\{\)\]\}]/)
  ...
  function autoCompletion(cm, caret, id, shiftKey) {
    ...
    getWords(1, id)
    if (
      (ssParam.queryFormer.toUpperCase().match(/^PREFIX\s+#$/) ||
        ssParam.queryFormer.toUpperCase().match(/^COPY\s+#$/)) &&
      ssParam.termFrag.match(/^#\d+$/) &&
      caret.line == 0
    ) {
  ...
  function autoCompletionTerms(id, shiftKey) {
    ...
    terms = unique(terms)
    const setTermFrag = ssParam.termFrag.toLowerCase()
    ...
    for (let i = 0; i < terms.length; i++) {
      const string = terms[i].toLowerCase()
      const stringPrefix = string.substr(0, setTermFrag.length)
      const stringNoBracket = string.replace(/[\s\(\[\{\)\]\}]/g, '')
  ...
  function autoCompletionPrefix(cm, caret, id) {
    const prefix = ssParam.termFrag.replace(/:$/, '')
  ...
  function autoCompletionUri() {
    const term = ssParam.termFrag.replace(/[<>]/g, '')
    let uri = false
  ...
  function autoCompletionEndpoint() {
    const domain = getDomain(ssParam.pathName)
    let string = '## endpoint '
    if (ssParam.termFrag == 'endpoint') string = 'endpoint '
    else if (ssParam.termFrag == 'http') string = ''
    let next = ''
    if (localStorage[domain + '_endpoint_list']) {
      const pre = ssParam.termFragTmp
      const endpoints = localStorage[domain + '_endpoint_list'].split(' ')
      if (!pre.match(/^https*:\/\//)) {
        next = endpoints[0]
      } else {
  ...
  function copyPrefix(cm, id) {
    const copy = ssParam.queryFormer.toUpperCase()
    const tarTab = parseInt(ssParam.termFrag.replace(/^#/, '')) - 1
    const selTab = parseInt(localStorage[ssParam.pathName + '_sparql_code_select_tab_' + id])
    const tabNum = parseInt(localStorage[ssParam.pathName + '_sparql_code_tab_num_' + id])
    let string = false
    if (copy.match(/PREFIX/)) {
      if (tarTab != selTab && tarTab <= tabNum) {
        string = ''
        const code = localStorage[ssParam.pathName + '_sparql_code_' + tarTab + '_' + id].split('\n')
  ...
  function getWords(allQueryFlag, id) {
    let text = ''
    if (allQueryFlag) {
      if (localStorage[ssParam.pathName + '_sparql_code_tab_num_' + id] === undefined)
        localStorage[ssParam.pathName + '_sparql_code_tab_num_' + id] = 0
      ...
      if (id.match(/^slsTa_/)) {
        // for SLARQList support
        const sparqlet = id.match(/^slsTa_(\w+)___/)[1]
        if (localStorage[ssParam.pathName + '_' + sparqlet + '_objects']) {
          const objects = localStorage[ssParam.pathName + '_' + sparqlet + '_objects'].split(' ')
          for (let i = 0; i < objects.length; i++) {
            const sls_id = 'slsTa_' + sparqlet + '___' + objects[i]
    ...
    const terms = unique(variable).sort()
    Array.prototype.push.apply(
      terms,
      Object.keys(freqPrefix).sort(function (a, b) {
    ...
    Array.prototype.push.apply(
      terms,
      Object.keys(freqTerm).sort(function (a, b) {
...
function chkQueryPrefix(id) {
  ...
  for (let i = 0; i < strings.length; i++) {
    ...
    for (let j = 0; j < words.length; j++) {
      if (words[j].match(/^\w*:/)) {
        ...
        const prefix = words[j].match(/^(\w*:)/)[1]
  ...
  if (Object.keys(unknownPrefix).length > 0) {
    ...
    ssParam.resultNode[id].getElementsByTagName('pre')[0] &&
...
async function innerModeRunQuery(queryTab, id, describe) {
  //// loading loop
  const loadingVis = function (runId, id) {
    const runTab = ssParam.job[runId]
    const selTab = parseInt(localStorage[ssParam.pathName + '_sparql_code_select_tab_' + id])
    ...
    if (document.getElementById('query_tab_' + runTab + '_' + id)) {
      document.getElementById('query_tab_' + runTab + '_' + id).style.borderColor =
        'rgba(255,' + ssParam.color.n + ',63,0.7)'

      if (
        runTab == selTab &&
        !document.getElementById('loadingIcon_' + runTab + '_' + id) &&
        !document.getElementById('slsLoading_' + id)
      ) {
        // for SPARQList support id
  ...
  const handleResponse = async function (res) {
    if (!res.ok) {
      const obj = { error: 1, status: res.status + ' ' + res.statusText }
      if (res.status == '405') {
        ...
        if (endpointLine && obj.text.match(/line \d+: syntax error /))
          obj.text = obj.text.replace(
            /line (\d+): syntax error /,
            'line ' + (parseInt(obj.text.match(/line (\d+): syntax error /)[1]) + 1) + ': syntax error '
          )
        ...
        if (defineLine) plus++
        if (plus && obj.text.match(/Parse error on line \d+:/))
          obj.text = obj.text.replace(
            /Parse error on line (\d+):/,
            'Parse error on line ' + (parseInt(obj.text.match(/Parse error on line (\d+):/)[1]) + plus) + ':'
          )
      }
      return obj
  ...
  const outError = function (res, runId) {
    const runTab = ssParam.job[runId]
    const pre = document.createElement('pre')
    pre.id = 'inner_result'
    ...
    } else if (document.getElementById('slsLoading_' + id)) {
      // for SPARQList support
  ...
  //download button
  const mkDlButton = function (resDiv, tab, id) {
    ...
    dlSelect.onchange = function () {
      ...
      if (type != 'dl') {
        const res = ssParam.dlResults['dl_json_' + tab + ' ' + id]
        ...
        if (type == 'csv' || type == 'tsv') {
          ...
          for (let i = 0; i < res.results.bindings.length; i++) {
            const values = []
            for (const param of res.head.vars) {
              let value = ''
  ...
  const outResult = function (res, runId, endpoint, describe) {
    ...
    const sec = Math.round((endTime - startTime) / 100) / 10
    resTime.appendChild(
      document.createTextNode('[ ' + res.results.bindings.length + ' bindings. -- ' + sec + ' sec. ]')
    )
    ...
    for (let i = 0; i < vars.length; i++) {
      const regex = new RegExp('\\s\\$' + vars[i] + '\\s')
    ...
    for (let i = 0; i < res.results.bindings.length; i++) {
      ...
      for (let j = 0; j < vars.length; j++) {
        ...
        if (res.results.bindings[i][vars[j]]) {
          const type = res.results.bindings[i][vars[j]].type
          const value = res.results.bindings[i][vars[j]].value
          if (type == 'uri') {
            let text = value
            let tmp = '$' + vars[j]
            if (searchPredicate && vars[j] == '__p__') tmp = '??'
            let term = tmp + ':<' + value + '>'
            for (const key in uri2prefix) {
    ...
    const resJson = document.createElement('pre')
  ...
  const runId = startTime + '_' + queryTab
  ...
  if (lines[0].toLowerCase().match(/^define +sql:select-option +"order"/) && lines[1].match(/^\s*#+.* *http/)) {
    // replace DEFINE-line to endpoint-line
    const tmp = lines[0]
    lines[0] = lines[1]
    lines[1] = tmp
    defineLine = true
  ...
  if (
    (lines[0] && lines[0].toLowerCase().match(/^\s*##+ *endpoint +https*:\/\//)) ||
    lines[0].match(/^\s*#+ *https*:\/\//)
  ) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      lines[i].match(/^# +@endpoint +https*:/) ||
      lines[i].match(/^# +@param +\w+/) ||
      lines[i].match(/^# +@temp-proxy +true/)
    )
      sparqlDoc = true
    ...
    if ((lines[i].match(/ \^*\?\?\s/) || lines[i].match(/ \^*\?\?$/)) && !lines[i].match(/^ *#/) && mlcF == 1)
      searchPredicate = true
    if (lines[i].match(/^\s*=begin\s*$/) || lines[i].match(/^\s*=b\s*$/)) {
      multiLineComment = true
      mlcF = 0
    } else if (lines[i].match(/^\s*=end\s*$/) || lines[i].match(/^\s*=e\s*$/)) {
      mlcF = 1
    }
  }

  const uri2prefix = {}
  for (const key in ssParam.prefix2Uri) {
  ...
  if (describe) {
    ...
    const sparqlMustacheParams = []
    ssParam.temporary_proxy = false
    for (let i = 0; i < lines.length; i++) {
      if (sparqlApiMd) {
        // for SPARQList markdown format
        if (f == 0 && lines[i].match(/^\* +\`\w+\`/)) {
          const key = lines[i].match(/^\* +\`(\w+)\`/)[1]
          const value = lines[i + 1].match(/^ +\* *default: *(.+)$/)[1]
          sparqlMustacheParams[key] = value
        }
        if (f == 0 && lines[i].match(/^\s*https*:\/\//)) {
          endpoint = lines[i].match(/^\s*(https*:\/\/[^\s;,]+)/)[1]
        } else if (lines[i].match(/^\`\`\`sparql/)) {
          f = 1
          continue
      ...
      if (sparqlDoc) {
        // for sparql-doc format
        if (lines[i].match(/^# +@param +.+/)) {
          const param_line = lines[i].match(/^# +@param +(\w+)\s*=\s*(.+)/)
          const key = param_line[1]
      ...
      if (sparqlMustache && (sparqlDoc || sparqlApiMd)) {
        // for mustache parameters
        while (lines[i].match(/\{\{\w+\}\}/)) {
          if (lines[i].match(/\{\{\w+\}\}/)) {
            const words = lines[i].match(/(.*)\{\{(\w+)\}\}(.*)/)
            if (sparqlMustacheParams[words[2]]) lines[i] = words[1] + sparqlMustacheParams[words[2]] + words[3]
          }
        }
      }
      if (searchPredicate && f == 1) {
        // for predicate seaech
        if (searchPredicate && lines[i].toUpperCase().match(/\s*SELECT /)) {
          lines[i] = '\nSELECT DISTINCT ?__p__ (SAMPLE(?__o__) AS ?sample)'
        } else if (searchPredicate && (lines[i].match(/ \?\?\s/) || lines[i].match(/ \?\?$/))) {
      ...
      if (multiLineComment) {
        // for multi-line comment out
        if ((lines[i].match(/^\s*=begin\s*$/) || lines[i].match(/^\s*=b\s*$/)) && f == 1) {
          f = 0
        } else if ((lines[i].match(/^\s*=end\s*/) || lines[i].match(/^\s*=e\s*$/)) && f == 0) {
          f = 1
          continue
        }
      }
  ...
  if (document.getElementById('default-graph-uri'))
    body += '&default-graph-uri=' + encodeURIComponent(document.getElementById('default-graph-uri').value)
  if (document.getElementById('timeout')) body += '&timeout=' + document.getElementById('timeout').value
  if (document.getElementById('debug')) {
    if (document.getElementById('debug').checked) body += '&debug=on'
  }
  const options = {
    method: 'POST',
    body: body,
  ...
  if (
    (ssParam.mixedContent == 1 && location.protocol == 'https:' && endpoint.match(/^http:/)) ||
    ssParam.temporary_proxy
  ) {
  ...
  //// run fetch
  try {
    const res = await timeout(600000, fetch(endpoint, options)).then(handleResponse)
    if (res.error) {
      outError(res, runId)
    } else {
...
function initDiv(cm, id) {
  ...
  if (document.getElementsByTagName('title')) {
    ...
    if (page_title == 'sparql-proxy' && document.getElementsByTagName('nav')) {
      const page_nav = document.getElementsByTagName('nav')[0].innerHTML
      if (page_nav.match('SPARQL Proxy')) ssParam.sparqlProxyFlag = true
    }
  }

  const codeMirrorDiv = cm.display.wrapper
  ...
  const controlNode = parentNode.insertBefore(newNode, codeMirrorDiv)
  ...
  const clipboardNode = parentNode.insertBefore(newNode, codeMirrorDiv.nextSibling)
  ...
  const cmStyle = document.getElementById('sparqlSupportCmDivStyle')
  let baseStyle = {},
    borderWidth = [],
    borderStyle = [],
    borderColor = [],
  ...
  if (!cmStyle) {
    ...
    borderWidth = [
      baseStyle.borderTopWidth,
      baseStyle.borderRightWidth,
      baseStyle.borderBottomWidth,
      baseStyle.borderLeftWidth
    ]
    borderStyle = [
      baseStyle.borderTopStyle,
      baseStyle.borderRightStyle,
      baseStyle.borderBottomStyle,
      baseStyle.borderLeftStyle
    ]
    borderColor = [
      baseStyle.borderTopColor,
      baseStyle.borderRightColor,
      baseStyle.borderBottomColor,
      baseStyle.borderLeftColor
    ]
    borderRadius = [
      baseStyle.borderTopLeftRadius,
      baseStyle.borderTopRightRadius,
      baseStyle.borderBottomRightRadius,
      baseStyle.borderBottomLeftRadius
    ]
    ...
    ssParam.areaDeltaWidth =
      parseInt(padding[1].replace(/px/, '')) +
      parseInt(padding[3].replace(/px/, '')) +
      parseInt(borderWidth[1].replace(/px/, '')) +
      parseInt(borderWidth[3].replace(/px/, ''))
    ssParam.areaDeltaHeight =
      parseInt(padding[0].replace(/px/, '')) +
      parseInt(padding[2].replace(/px/, '')) +
      parseInt(borderWidth[0].replace(/px/, '')) +
      parseInt(borderWidth[2].replace(/px/, ''))
    areaWidth = width - ssParam.areaDeltaWidth
    areaHeight = height - ssParam.areaDeltaHeight
  } else {
    const a = cmStyle.value.split('|')
    areaHeight = a[1]
    areaWidth = a[2]
  ...
  if (
    ...
    baseStyle.fontFamily != 'Monaco'
  )
    codeMirrorDiv.style.fontFamily = 'monospace, Courier, Consolas, Monaco'
  ...
  if (localStorage[ssParam.pathName + '_prefix_uri_' + id]) {
    const list = localStorage[ssParam.pathName + '_prefix_uri_' + id].split(' ')
    const time = list.shift()
  ...
  if (
    localStorage[ssParam.pathName + '_sparql_code_' + selTab + '_' + id] != null &&
    localStorage[ssParam.pathName + '_sparql_code_' + selTab + '_' + id].match(/\w/)
  ) {
  ...
  if (localStorage[ssParam.pathName + '_sparql_term_freq']) {
    const array = localStorage[ssParam.pathName + '_sparql_term_freq'].split(' ')
  ...
  if (param_code) {
    ...
    if (localStorage[path + '_sparql_code_tab_num_query']) {
      let tabNum = parseInt(localStorage[path + '_sparql_code_tab_num_query'])
      for (let i = 0; i <= tabNum; i++) {
        if (typeof localStorage[path + '_sparql_code_' + i + '_query'] == 'undefined') {
          ...
          localStorage[path + '_sparql_code_' + i + '_query'].replace(/[\n\r]+/g, '\n') ==
          param_code.replace(/[\n\r]+/g, '\n')
        ) {
  ...
  const resParent = document.createElement('div')
  ...
  const confirm = document.createElement('ul')
  ...
  if (ssParam.userAgent == 'webkit') {
    // for edit-area resize in webkit
    const cmS = document.getElementsByClassName('CodeMirror-scroll')
  ...
  if (!id.match(/^slsTa_/)) {
    ...
    document.getElementById('rawQueryRadio').onclick = function () {
      showAutoRunBox(0)
    }
    document.getElementById('queryUrlRadio').onclick = function () {
      showAutoRunBox(1)
    }
    document.getElementById('shortUrlRadio').onclick = function () {
      showAutoRunBox(2)
    }
    document.getElementById('autoRunChkBox').onclick = function () {
      setShortUrl()
    }
  }
...
function initDivQueries(cm, id) {
  const codeMirrorDiv = ssParam.codeMirrorDiv[id]
  const ulNode = document.getElementById('query_tab_list_' + id)
  if (!ssParam.results) ssParam.results = {}

  let tabNum = 0
  let selTab = 0
  if (localStorage[ssParam.pathName + '_sparql_code_tab_num_' + id])
...
function initDivInner(cm, id) {
  const codeMirrorDiv = ssParam.codeMirrorDiv[id]
  const ulNode = document.getElementById('query_tab_list_' + id)
  const ulNodeHelp = document.getElementById('query_help_' + id)
  let liNode = document.createElement('li')
  if (!ssParam.innerMode) ssParam.innerMode = {}
  ssParam.innerMode[id] = 0

  const moveSwitchTab = function (hideNode, showNode) {
    let count = 0
    const moveTab = function () {
      count++
      if (count <= 10) {
        const tmp = count * -3
        hideNode.style.top = tmp + 'px'
      } else if (count <= 20) {
        const tmp = (count - 10) * 3 - 25
        showNode.style.top = tmp + 'px'
      } else clearInterval(timer)
    }
    const timer = setInterval(moveTab, 30)
  }
  ...
  if (window.navigator.languages && window.navigator.languages[0] == 'ja')
    url = 'https://sparql-support.dbcls.jp/sparql-support_j.html'
  ...
  if (localStorage[ssParam.pathName + '_sparql_target_' + id]) {
    const target = localStorage[ssParam.pathName + '_sparql_target_' + id]
    if (target == 'inner') {
      switchInnerMode(id)
    } else if (target == 'inner_j') {
      switchInnerMode(id)
      switchInnerMode(id)
    } else if (ssParam.sparqlProxyFlag) {
      switchInnerMode(id)
    }
  } else {
    switchInnerMode(id)
...
function addTab(cm, id) {
  const tabNum = parseInt(localStorage[ssParam.pathName + '_sparql_code_tab_num_' + id]) + 1
  const liNode = document.createElement('li')
  const plusNode = document.getElementById('query_tab_list_' + id).childNodes[tabNum]
...
function removeTabRun(cm, id) {
  ...
  for (let i = selTab; i < tabNum; i++) {
    ...
    if (localStorage[ssParam.pathName + '_sparql_code_' + nextTab + '_' + id]) {
      localStorage[ssParam.pathName + '_sparql_code_' + i + '_' + id] =
        localStorage[ssParam.pathName + '_sparql_code_' + nextTab + '_' + id]
  ...
  const ulNode = document.getElementById('query_tab_list_' + id)
  const liNodes = ulNode.childNodes
  ...
  if (ssParam.results['sparql_res_' + selTab + '_' + id]) {
    const resTable = ssParam.results['sparql_res_' + selTab + '_' + id]
...
function changeTab(cm, newTab, id) {
  ...
  if (localStorage[ssParam.pathName + '_sparql_code_' + newTab + '_' + id])
    ssParam.textarea[id].value = localStorage[ssParam.pathName + '_sparql_code_' + newTab + '_' + id]
  else ssParam.textarea[id].value = ''

  ssParam.resultNode[id].innerHTML = ''
  ssParam.subResNode[id].style.display = 'none'
  if (ssParam.results['sparql_res_' + newTab + '_' + id]) {
    const resTable = ssParam.results['sparql_res_' + newTab + '_' + id]
    ssParam.resultNode[id].appendChild(resTable)
...
function replaceTab(cm, id, move) {
  const selTab = parseInt(localStorage[ssParam.pathName + '_sparql_code_select_tab_' + id])
  const tabNum = parseInt(localStorage[ssParam.pathName + '_sparql_code_tab_num_' + id])
  const targetTab = selTab + move
  if (targetTab >= 0 && targetTab <= tabNum) {
    if (!localStorage[ssParam.pathName + '_sparql_code_' + selTab + '_' + id])
      localStorage[ssParam.pathName + '_sparql_code_' + selTab + '_' + id] = ''
    if (!localStorage[ssParam.pathName + '_sparql_code_' + targetTab + '_' + id])
      localStorage[ssParam.pathName + '_sparql_code_' + targetTab + '_' + id] = ''
    ...
    const jobs = Object.keys(ssParam.job)
...
function switchInnerMode(id) {
  if (ssParam.resultNode[id].style.display == 'none') {
    ...
    for (let j = 0; tags[j]; j++) {
      const inputNodes = document.getElementsByTagName(tags[j])
    ...
    if (ssParam.innerMode[id] == 1) {
      ...
      ssParam.resultNode[id].style.display = 'none'
      const inputNode = document.getElementById('submit_button_' + id)
...
function setQueryTabWidth(tabNum) {
  ...
  const list = document.getElementsByClassName('query_tab')
  for (const i in list) {
...
function makeSparqlQueryLink() {
  ...
  const url = location.href.match(/([^\?]+)/)[1]
...
function copyToClipboard() {
  ...
  const timer = setInterval(anime, 10)
}

async function popupCopyForm() {
  const popup = document.getElementById('popupCopy')
...
async function setShortUrl() {
  const string = makeSparqlQueryLink()
  const json = await fetch('https://is.gd/create.php?format=json&url=' + encodeURIComponent(string)).then((res) =>
    res.json()
  )
...
function ssCommand(cm, id, caret, line) {
  const selTab = parseInt(localStorage[ssParam.pathName + '_sparql_code_select_tab_' + id])
  if ((line.match(/^#\s*clear_sparql_queries\s*;\s*$/) || line.match(/^#\s*バルス\s*;\s*$/)) && caret.line == 0) {
    // clear all
    localStorage.clear()
    ssParam.textarea[id].value = ''
    location.reload()
  } else if (line.match(/^#\s*font-size\s*:\s*\d+.*\s*;$/)) {
    //change font-size
    ...
    changeTab(cm, selTab, id)
    setCmDiv(cm, id)
  } else if (line.match(/^#\s*font-family\s*:\s*.+\s*;$/)) {
    //change font-family
    const family = line.match(/^#\s*font-family\s*:\s*(.+)\s*;\s*$/)[1]
    ...
    const string = autoCompletionSpang(line, id)
    caret = cm.getCursor('anchor')
    const end = cm.lineCount()
    cm.replaceRange(string, Pos(caret.line, caret.ch), Pos(end, 0))
  } else if (line.match(/^#\s*mixed-content-proxy\s*:\s*.+\s*;$/)) {
    const f = line.match(/^#\s*mixed-content-proxy\s*:\s*(.+)\s*;\s*$/)[1]
    if (f == 'true') {
  ...
  function autoCompletionSpang(spangLine) {
    ...
    const spang = spangLine.split(/\s+/)
    for (let j = 0; j < spang.length; j++) {
      if (spang[j].match('-S')) {
        const tmp = uriCheck(spang, j + 1)
        if (tmp.match(/^[^\-]/)) s = tmp
      } else if (spang[j].match('-P')) {
        const tmp = uriCheck(spang, j + 1)
        if (tmp.match(/^[^\-]/)) p = tmp
      } else if (spang[j].match('-O')) {
        const tmp = uriCheck(spang, j + 1)
        if (tmp.match(/^[^\-]/)) o = tmp
      } else if (spang[j].match('-F')) {
        const tmp = uriCheck(spang, j + 1)
        if (tmp.match(/^[^\-]/)) from = 'FROM ' + tmp
      } else if (spang[j].match('-L')) {
        let tmp = spang[j + 1]
        if (tmp.match(/[^\d]/)) {
          tmp = '10'
        }
        limit = 'LIMIT ' + tmp
      } else if (spang[j].match('-N')) {
        count = true
      } else if (spang[j].match('-G')) {
        graph = true
      }
    }
    let code = s + ' ' + p + ' ' + o + ' .'
...
function setDefaultUri() {
  const uri = { id: 'http://identifiers.org/' }
  ssParam.defaultUri = {}
  for (const key in ssParam.prefix2Uri) {
...
async function setPrefixUri(cm, caret, id, prefix, flag) {
  const url = 'https://prefix.cc/' + prefix + '.file.json'
  const options = { method: 'GET' }
  const res = await fetch(url, options).then((res) => res.json())
  const prefixes = prefix.split(',')
  for (let i = 0; i < prefixes.length; i++) {
    const prefix = prefixes[i].replace(/:$/, '')
    const uri = await res[prefix]
...
async function setFormAction(cm, id, text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].toLowerCase().match(/^# +@endpoint +https*:\/\/[^\s]/)) {
      ...
      const caret = cm.getCursor('anchor')
