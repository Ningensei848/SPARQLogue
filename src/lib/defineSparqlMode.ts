...
const defineSparqlMode = (config: CodeMirror.EditorConfiguration): CodeMirror.Mode<any> => {
  ...
  function tokenBase(stream: CodeMirror.StringStream, state: any) {
    ...
    if (sparql_doc == 2) {
      ...
      if (sparql_doc == 1) {
        if (stream.string.substr(stream.start).match(/ [^\s]/)) {
          sparql_doc = 2
        } else {
          sparql_doc = 0
        }
  ...
  function tokenComment(stream: CodeMirror.StringStream, state: any): string {
    let ch
  ...
  function pushContext(state: any, type: string, col: number): void {
    state.context = {
      prev: state.context,
      indent: state.indent,
      col: col,
  ...
  function popContext(state: any): void {
      ...
      if (curPunc == null) {
        ...
        if (/[}\]]/.test(state.context.type)) {
          ...
          if (
            ...
            const tempMatch01 = stream.string.match(/^(\s*)/)
            if (tempMatch01 != null) {
              state.context.col = tempMatch01[1].length
            }
            const tempMatch02 = stream.string.match(/^(.*\[\s*)[^\s]+\s+[^\s]+\s*;/)
            if (stream.string.match(/\[\s*[^\s]+\s+[^\s]+\s*;/) && tempMatch02) {
